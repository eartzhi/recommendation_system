...
Исходные данные предсталены четыремя файлами с набором данных:  
...
После импорта и объединения двух частей файла item_properties получили следующие датасеты:  
...
Для модели KNNWithMeans из библиотеки surprice были проведены эксперименты на следующей выборке гиперпараметров:  
...
Для модели LightFM из библиотеки lightfm были проведены эксперименты на следующей выборке гиперпараметров:  
...
Для модели XGBClassifier из библиотеки xgboost были проведены эксперименты на следующей выборке гиперпараметров:  
...
Для модели SVD из библиотеки surprice были проведены эксперименты на следующей выборке гиперпараметров:  
...
ENV RES_PASSWORD=password2  
Переменная окружения для определения пароля для запроса переобучения модели рекомендательной системы. При необходимости значение переменной можно изменить.  

Остальные параметры изменять не рекомендуется.  

Сборка образа осуществляется командой:  
docker build --no-cache -t recomendation_server path   
path-путь к dockerfile на хост машине.  
